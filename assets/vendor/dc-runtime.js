...
(() => {
  ...
  function createExternalModules(onResolved) {
    ...
    function ensureBabel() {
      ...
      babelLoading = new Promise((res, rej) => {
        ...
        const timer = setTimeout(() => rej(new Error(`timed out loading ${babel.src}`)), 1e4);
        s.onload = () => { clearTimeout(timer); res(); };
        s.onerror = () => { clearTimeout(timer); rej(new Error(`failed to load ${babel.src}`)); };
  ...
  function loadScript(src, integrity) {
    return new Promise((resolve2, reject) => {
      ...
      s.async = false;
      const timer = setTimeout(() => reject(new Error(`timed out loading ${src}`)), 1e4);
      s.onload = () => { clearTimeout(timer); resolve2(); };
  ...
  function init() {
    ...
    const api = {
      ...
      __dcBoot: () => {
        const booted = boot(runtime, document);
        if (booted) rootName = booted;
        else console.error("[dc] boot() found no <x-dc> root element — the app cannot render.");
        notifyHost();
      },
  ...
  loadReactUmd().then(init).catch((err) => {
    ...
    try {
      const boot = document.getElementById("__miniapp_boot");
      if (boot && boot.isConnected) {
        boot.classList.add("is-delayed");
        const text = boot.querySelector(".miniapp-boot__text");
        if (text) text.textContent = "Не удалось загрузить приложение. Проверьте интернет и попробуйте снова.";
      }
    } catch (e) {
    }
    throw err;
